...
/**
 * @summary Adds interceptors to the list of handlers to execute before each
 * request made by this replication.
 * @param {(...HttpInterceptor|HttpInterceptor[])} interceptors - The
 * interceptors to add. Interceptors are executed in a pipeline and modify the
 * connection context in a serial fashion.
 * @returns The replication builder instance
 */
ReplicatorBuilder.prototype.addRequestInterceptors = function() {
  ...
  interceptors.forEach(function(interceptor, index) {
    if (!_.isFunction(interceptor)) {
      throw new Error('Request interceptor at index ' + index +
      ' must be a function');
    }
  });
 ...
 * @param {ReplicatorBuilder~buildCallback} [callback] - The function to call
 * after attempting to build the {@link Replicator}.
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning
 * either the built {@link Replicator} or an Error.
 */
