...
#define TestCase(NAME)                                                \
    ...
    {                                                                 \
        linkToTest##NAME.next = gAllTestCases;                        \
        gAllTestCases = &linkToTest##NAME;                            \
    }                                                                 \
    void Test_##NAME(void)
#else
...
#define Assert(COND, MSG...)                                                   \
    do {                                                                       \
        if (__builtin_expect(!(COND), NO)) {                                   \
            _AssertFailed(self, _cmd, __FILE__, __LINE__, #COND, ##MSG, NULL); \
        }                                                                      \
    } while (0)
#define CAssert(COND, MSG...)                                                  \
    do {                                                                       \
        if (__builtin_expect(!(COND), NO)) {                                   \
            ...
            _AssertFailed(nil, _name, __FILE__, __LINE__, #COND, ##MSG, NULL); \
        }                                                                      \
    } while (0)

// AssertEqual is for Obj-C objects
#define AssertEqual(VAL, EXPECTED)                                                   \
    do {                                                                             \
        id _val = VAL, _expected = EXPECTED;                                         \
        Assert(_val == _expected || [_val isEqual:_expected],                        \
               @"Unexpected value for %s: %@ (expected %@)", #VAL, _val, _expected); \
    } while (0)
#define CAssertEqual(VAL, EXPECTED)                                                   \
    do {                                                                              \
        id _val = (VAL), _expected = (EXPECTED);                                      \
        CAssert(_val == _expected || [_val isEqual:_expected],                        \
                @"Unexpected value for %s: %@ (expected %@)", #VAL, _val, _expected); \
    } while (0)

// AssertEq is for scalars (int, float...)
#define AssertEq(VAL, EXPECTED)                                                       \
    do {                                                                              \
        __typeof(VAL) _val = VAL;                                                     \
        __typeof(EXPECTED) _expected = EXPECTED;                                      \
        Assert(_val == _expected, @"Unexpected value for %s: %@ (expected %@)", #VAL, \
               $object(_val), $object(_expected));                                    \
    } while (0)
#define CAssertEq(VAL, EXPECTED)                                                       \
    do {                                                                               \
        __typeof(VAL) _val = VAL;                                                      \
        __typeof(EXPECTED) _expected = EXPECTED;                                       \
        CAssert(_val == _expected, @"Unexpected value for %s: %@ (expected %@)", #VAL, \
                $object(_val), $object(_expected));                                    \
    } while (0)
