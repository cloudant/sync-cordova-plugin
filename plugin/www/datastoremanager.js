 ...
 * @description If there is no existing {@link Datastore} with the given name on disk then one will be created.
 * If an encryptionOptions object is supplied, the {@link Datastore} will be encrypted.
 *
 * @function DatastoreManager#openDatastore
 * @param {String} name - The name of the {@link Datastore} to open
 * @param {Object} [encryptionOptions] - Options used for encrypting the {@link Datastore}.
 * @param {String} encryptionOptions.password - A user-provided password.
 * @param {String} encryptionOptions.identifier - A unique identifier for retrieving the encryption key.
 * @param {DatastoreManager~openDatastoreCallback} [callback] - The function to call after attempting to open the Datastore.
 *
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either a {@link Datastore} or an Error.
 *
 */
exports.openDatastore = function(name, encryptionOptions, callback) {
    ...
    if ((!_.isEmpty(encryptionOptions.password) && _.isEmpty(
        ...
        throw new Error(
            'Both password and identifier must be specified or neither must be specified'
        );
    }
 ...
 * @description Deletes a {@link Datastore}'s files from disk; data replicated to remote databases is not affected.
 *
 * @function DatastoreManager#deleteDatastore
 * @param {String} name The name of the {@link Datastore} to delete
 * @param {DatastoreManager~deleteDatastoreCallback} [callback] The function to call after attempting to delete the {@link Datastore}.
 *
...
/**
 * @class Datastore
 * @classdesc The {@link Datastore} prototype is a common interface for create, read, update and delete (CRUD)
 * operations for within Cloudant Sync.
 * @property {String} name - The {@link Datastore} name (readonly)
 *
 * @description <strong>Should not be called by user; Use {@link DatastoreManager#openDatastore} to get {@link Datastore} objects</strong>
 */
 ...
 * @description If '_id' in the revision is null or undefined, the document's '_id' will be auto-generated
 ...
 * @param {Datastore~createDocumentFromRevisionCallback} [callback] - The function to call after attempting to create the document.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the saved document revision or an Error.
 */
Datastore.prototype.createDocumentFromRevision = function(documentRevision, callback) {
    if (!documentRevision)
        throw new Error('documentRevision must exist');

    if (!_.isObject(documentRevision) || _.isArray(documentRevision) || _.isFunction(documentRevision))
...
/**
 * @summary Updates a document that exists in the datastore with body and attachments from revision.
 ...
 * @param {Datastore~updateDocumentFromRevisionCallback} [callback] - The function to call after attempting to update the document.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the saved document revision or an Error.
 */
Datastore.prototype.updateDocumentFromRevision = function(documentRevision, callback) {
    if (!documentRevision)
        throw new Error('documentRevision must exist');
 ...
 * @param {Datastore~getDocumentCallback} [callback] - The function to call after attempting to fetch the document.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the retrieved document revision or an Error.
 */
Datastore.prototype.getDocument = function(documentId, callback) {
    ...
    exec(successHandler, errorHandler, "CloudantSync", "getDocument", [this.name, documentId]);
 ...
 * @description This operation leaves a "tombstone" for the deleted document so that future replication operations
 * can successfully replicate the deletion. If the input revision is already deleted, nothing
 * will be changed.
 *
 * @param {Object} documentRevision - The document revision to delete.
 * @param {Datastore~deleteDocumentFromRevisionCallback} [callback] - The function to call after attempting to delete the document.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the deleted document revision or an Error.
 */
Datastore.prototype.deleteDocumentFromRevision = function(documentRevision, callback) {
    ...
    if (!errorMessage) {
        exec(successHandler, errorHandler, "CloudantSync", "deleteDocumentFromRevision", [this.name, documentRevision]);
    } else {
 ...
 * @param {Datastore~ensureIndexedCallback} [callback] - The function to call after attempting to create the index.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the created index name or an Error.
 */
Datastore.prototype.ensureIndexed = function(fieldNames, indexName, callback) {
    ...
    exec(successHandler, errorHandler, "CloudantSync", "ensureIndexed", [this.name, indexName, fieldNames]);
 ...
 * @param {Datastore~deleteIndexNamedCallback} [callback] - The function to call after attempting to delete the index.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the deletion status [true|false] or an Error.
 */
Datastore.prototype.deleteIndexNamed = function(indexName, callback) {
    ...
    exec(successHandler, errorHandler, "CloudantSync", "deleteIndexNamed", [this.name, indexName]);
 ...
 * @param {Datastore~findCallback} [callback] - The function to call after attempting to perform a query.
 *
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either an Array of the query results or an Error.
 */
Datastore.prototype.find = function(query, callback) {
    ...
    exec(successHandler, errorHandler, "CloudantSync", "find", [this.name, query]);
 ...
 * @param {Datastore~createDocumentFromRevisionCallback | Datastore~updateDocumentFromRevisionCallback} [callback] - The function to call after attempting to save the document.
 * @returns A [q style promise]{@link https://github.com/kriskowal/q} returning either the saved document revision or an Error.
 */
function save(dbName, documentRevision, callback) {
    ...
    if (documentRevision._attachments) {
        ...
        if (_.isEmpty(attachments)) {
            throw new Error(
                'documentRevision contained invalid attachments.  _attachments had no body'
            );
        }
    ...
    if (!errorMessage) {
        exec(successHandler, errorHandler, "CloudantSync", "save", [dbName, documentRevision]);
    } else {
        deferred.reject(new Error(errorMessage));
    }
